...
(function (exports, undefined) {
  ...
  var А = (w.Ρ = w.Ρ || {});
  А.Η = ["touches", "changedTouches", "targetTouches"];
  А.touches = "touches";
...
E.LW = {
  ...
  _B: {
    ...
    f1: function (y) {
      if (y === 0) return 0;
      if (y === 1) return 1;
      if ((y *= 2) < 1) return 0.5 * Math.pow(1024, y - 1);
  ...
  Ζs: {
    ...
    f1: function (_) {
      if ((_ *= 2) < 1) return -0.5 * (Math.sqrt(1 - _ * _) - 1);
  ...
  vМ: {
    nD: function (Ε) {
      ...
      if (!Y || Y < 1) {
        Y = 1;
        G = І / 4;
      } else G = (І * Math.asin(1 / Y)) / (2 * Math.PI);
      return -(
        Y *
        Math.pow(2, 10 * (Ε -= 1)) *
        Math.sin(((Ε - G) * (2 * Math.PI)) / І)
      );
    },
    Mu: function (Η) {
      var О,
      ...
      if (!z || z < 1) {
        z = 1;
        О = Υ / 4;
      } else О = (Υ * Math.asin(1 / z)) / (2 * Math.PI);
      return (
        z * Math.pow(2, -10 * Η) * Math.sin(((Η - О) * (2 * Math.PI)) / Υ) + 1
      );
    },
    f1: function (G) {
      var f,
      ...
      if (!Р || Р < 1) {
        Р = 1;
        f = z / 4;
      } else f = (z * Math.asin(1 / Р)) / (2 * Math.PI);
      if ((G *= 2) < 1)
        return (
          -0.5 *
          (Р *
            Math.pow(2, 10 * (G -= 1)) *
            Math.sin(((G - f) * (2 * Math.PI)) / z))
        );
      return (
        Р *
          Math.pow(2, -10 * (G -= 1)) *
  ...
  rT: {
    ...
    f1: function (w) {
      var V = 1.70158 * 1.525;
      if ((w *= 2) < 1) return 0.5 * (w * w * ((V + 1) * w - V));
  ...
  FF: {
    ...
    f1: function (I) {
      if (I < 0.5) return E.LW.FF.nD(I * 2) * 0.5;
...
E.kl = {
  _Ѕ: function (І, w) {
    var Q = І.length - 1,
      ...
      X = Math.floor(Ρ),
      g = E.kl.ΚΝ._Ѕ;
    if (w < 0) return g(І[0], І[1], Ρ);
    if (w > 1) return g(І[Q], І[Q - 1], Q - Ρ);
  ...
  sa: function (М, H) {
    var o = М.length - 1,
      ...
      X = E.kl.ΚΝ.sa;
    if (М[0] === М[o]) {
      if (H < 0) z = Math.floor((Κ = o * (1 + H)));
      return X(М[(z - 1 + o) % o], М[z], М[(z + 1) % o], М[(z + 2) % o], Κ - z);
    } else {
      if (H < 0) return М[0] - (X(М[0], М[0], М[1], М[1], -Κ) - М[0]);
      if (H > 1)
...
var t = {
  ...
  cЕ: function (T, Β, h) {
    Β = Β || 0;
    h = h || 0;
    return [Β, h, Β + T.width, h + T.height];
  },
  ...
  kΤ: function () {
    ...
    var Κ =
      ...
      /(safari)[ \/]([\w.]+)/.exec(H) ||
      /(webkit)[ \/]([\w.]+)/.exec(H) ||
      (!/compatible/.test(H) && /(mozilla)(?:.*? rv:([\w.]+))?/.exec(H)) ||
      [];
    Ј[Κ[1]] = true;
...
(function (exports, undefined) {
  ...
  Player.prototype = {
    ...
    resize: function () {
      ...
      this.ΟΤ =
        this.tt + ((this.ZР.height - 100 - this.tt - this.Ϲ1.height) >> 1);
    },
    OА: function (O) {
      if (this.OA) {
        return;
...
(function (exports, undefined) {
  ...
  Photo.prototype = {
    ...
    PΝ: function (T, I, Τ) {
      ...
      T.drawImage(
        this.canvas,
        this.ox + this.borderWidth,
        this.oy + this.borderWidth,
